// agents/generalChatAgent.js
const { sendTextMessage } = require('../utils/whatsappApi');
const { GoogleGenerativeAI } = require('@google/generative-ai');
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" }); // Can use a more conversational model if needed

async function handleGeneralChat(senderWaId, messageContent) {
    console.log(`[GeneralChatAgent] Handling general chat from ${senderWaId}: "${messageContent}"`);
    ...
    try {
        ...
        console.error("Error generating general chat response with Gemini:", error);
